...
// invert black and white only colors set to background color
void effect_invert_bw_with_background(GContext* ctx,  GRect position, void* param) {
  GBitmap *fb = graphics_capture_frame_buffer(ctx); //capturing framebuffer bitmap
  ...
  for (int y = 0; y < position.size.h; y++) {
     for (int x = 0; x < position.size.w; x++) {
          ...
          if (gcolor_equal(pixel, GColorBlack))
            ...
            set_pixel(bitmap_info, y + position.origin.y, x + position.origin.x, (uint8_t)((int32_t)param)); // don't ask...
     }
  }
  graphics_release_frame_buffer(ctx, fb);
}
